...
function extract_Category_From_Request(req) {
  // .../api/ressources/category
  let parts = req.url.split("/");
...
async function handleBookmarksServiceRequest(req, res) {
  if (req.url.includes("/api/bookmarks")) {
    ...
    let validStatus = "";
    let id = extract_Id_From_Request(req);
    let category = extract_Category_From_Request(req).normalize('NFD').replace(/[\u0300-\u036f]/g, '').replaceAll(' ', '');

    //RETIRER LES ACCENTS QUAND ON VÉRIFIE LE LIEN
    switch (req.method) {
      case "GET":
        if (isNaN(id) && category == "bookmarks") {
          res.writeHead(200, { "content-type": "application/json" });
          ...
          if (isNaN(id)) {
            ...
            for(let bookmark of bookmarks){
                console.log(bookmark.Category.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replaceAll(' ', ''));
                if(bookmark.Category.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replaceAll(' ', '') == category){
                    filteredBookmarks.push(bookmark)
                }
            }
            if(filteredBookmarks.length <1){
                res.writeHead(404);
                res.end(`Error : The bookmarks from ${category} do not exist`);  
            }
            else{
                res.writeHead(200, {"content-type": "application/json"});
                res.end(JSON.stringify(filteredBookmarks));
            }
          } else {
            let found = false;
            for (let bookmark of bookmarks) {
